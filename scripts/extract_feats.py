...
def main():
    ...
    parser.add_argument("-i", "--input_dir", default="data/PHON/hillenbrand/audio/", type=Path)
    ...
    # read and merge dataframes from hillenbrand
    merged_df = merge_tsv_files('data/PHON/hillenbrand/bigdata.tsv', 'data/PHON/hillenbrand/timedata.tsv')

    # Check wav files in input directory and keep those in merged df
    wav_paths = list(args.input_dir.glob("*.wav"))
    wav_paths = [w for w in wav_paths if str(w).split('/')[-1][:-4] in merged_df['file'].to_list()]
